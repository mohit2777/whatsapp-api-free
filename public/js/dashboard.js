...
// API Documentation View - Uses the pre-built HTML section
function showApiDocsView() {
    ...
    // Show API Docs view
    if (apiDocsView) {
        apiDocsView.style.display = 'block';
...
function copyCode(button) {
    const codeBlock = button.closest('.docs-code-block') || button.closest('div');
...
// Setup API Docs Tester
function setupApiDocsTester() {
    const runTestBtn = document.getElementById('runApiTest');
    if (runTestBtn && !runTestBtn.hasAttribute('data-initialized')) {
        runTestBtn.setAttribute('data-initialized', 'true');
        runTestBtn.addEventListener('click', async () => {
            ...
            runTestBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Sending...';
            ...
            try {
                ...
                if (resultContainer && outputPre) {
                    resultContainer.style.display = 'block';
                    outputPre.textContent = JSON.stringify(result, null, 2);
                    outputPre.style.color = response.ok ? '#4ade80' : '#ef5350';
                }
                ...
                runTestBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Send Test Message';
            }
        });
    }
}
...
function toggleApiEndpoint(header) {
    const card = header.closest('.api-endpoint, .docs-endpoint');
    const content = card.querySelector('.endpoint-content, .docs-endpoint-content');
